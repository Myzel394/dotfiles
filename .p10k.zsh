# Generated by Powerlevel10k configuration wizard on 2023-10-27 at 23:14 CEST.
...
# If in the current working directory there is a `docker-compose.yaml` file,
# then show the name of the running docker container.
function prompt_docker_compose_info() {
    # To speed up the process, check if there is a file named `docker-compose.yaml` in the pwd
    # Accept both `yaml and `yml` extensions.
    if [[ ! -f "docker-compose.yaml" && ! -f "docker-compose.yml" ]]; then
        return
    fi

    available_containers=$(docker-compose ps -q 2>/dev/null | wc -l)

    if [[ -z "$available_containers" ]]; then
        return
    fi

    running_containers=$(docker-compose ps | awk '{if ($4 == "Up") print $0}' | wc -l  )

    p10k segment -b black -f '#0db7ed' -i "🐳" -t "${running_containers}/${available_containers}"
}
...
() {
  ...
  # Nvm color.
  typeset -g POWERLEVEL9K_NVM_FOREGROUND=253
...
function prompt_initial_loading() {}

function instant_prompt_initial_loading() {
  p10k segment -b red -f 254 -t "loading..."
}
